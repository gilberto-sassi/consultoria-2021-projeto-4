...
df_cvc <- df_cvc %>% 
  tidyr::pivot_longer(cols = tidyselect::starts_with("CVC"),
                    names_to = "Divisão", values_to = "CVC") %>%
...
ggplot2::ggplot(df_cvc, ggplot2::aes(x = ItemNum)) +
  ggplot2::geom_bar(ggplot2::aes(y = CVC, fill = `Divisão`, color = `Divisão`),
           ...
           width = 0.5, size = 0.1) +
  ggplot2::geom_line(data = df_hline,
                    ggplot2::aes(x = x, y = y, linetype = "corte"),
            size = 0.5, color = "darkblue") +
  ggplot2::labs(x = "Item") +
  ggplot2::scale_fill_manual(values = c("CVC -- clareza e compreensão" = "gray",
                                      "CVC -- relevância" = "orange"),
                    labels = legenda_fill) +
  ggplot2::scale_color_manual(values =
                                c("CVC -- clareza e compreensão" = "blue",
...
ggplot2::ggplot(df_cvc, ggplot2::aes(x = ItemNum)) +
  ...
  ggplot2::scale_fill_manual(values =
                                c("CVC -- clareza e compreensão" = "gray",
                                "CVC -- relevância" = "orange"),
                    labels = legenda_fill) +
  ggplot2::scale_color_manual(values =
                                c("CVC -- clareza e compreensão" = "blue",
